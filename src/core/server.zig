...
pub fn run(init: process.Init) !void {
    ...
    const fp_result = try chromaprint.calcFingerprint("assets/Jane!.opus", &.{});
    defer chromaprint.chromaprintDealloc(@constCast(fp_result.fingerprint.?.ptr));
    const took = start.durationTo(Io.Clock.awake.now(io));
    std.debug.print("fp: {?s}\nduration: {f}\ntook: {f}\n", .{fp_result.fingerprint, fp_result.duration, took});
    ...
    const result = try lib.acoustid.lookupByFingerprint(&client, gpa, .{ 
        ...
        .duration = fp_result.duration.toSeconds(),
        .meta = &.{
            .recordings, 
            .recordingids,
            .releases,
    ...
    var mbid: ?lib.music_brainz.MBID = null;
    ...
    if (mbid) |mbid_p| {
        const entity: lib.music_brainz.Entity = .{
            .recording = .{ 
                .incl = .{ 
                    .misc = .{ 
                        .aliases = true,
        ...
        const mb_result = try lib.music_brainz.lookupEntityValue(&client, gpa, &entity);
        std.debug.print("music_brainz result: {any}\n", .{mb_result.value});
    } else {
        std.debug.print("no mbid\n", .{});
    }
}
